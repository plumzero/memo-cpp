...
适用于:
- 包含基本 C/C++ 数据类型之间的转换(不能用于指针之间，可以使用 reinterpret_cast)
- 具有继承关系的类型之间的转换(指针与指针、引用与引用之间的向上类型的安全转换)

### dynamic_cast

适用于:
- 继承关系中，将父类对象的指针或引用转换为子类对象的指针或引用(向下类型转换)
...
将一个类型的对象重新强制解释成另一个类型的对象。其操作语义是对指针所指向内存的二进制数据的纯粹拷贝，所以其可应用于互不相差类型指针之间的转换。也可用于指针与整型之间的转换，此时的整型应为特定平台下某个指针的整型值表示。
...
将常量型对象转换成对应的非常量型对象(基本类型、类对象、指针均适用)。
