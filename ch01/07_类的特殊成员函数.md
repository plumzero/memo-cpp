...
| 左值拷贝函数  | 拷贝构造函数和拷贝赋值操作符的统称 |
| 右值移动函数  | 移动构造函数和移动赋值操作符的统称  |
...
| 析构函数         | C::~C();                |
| 拷贝构造函数     | C::C(const C&);          |
| 拷贝赋值操作符   | C& operator=(const C&);  |
| 移动构造函数     | C::C(C&&);               |
| 移动赋值操作符   | C& operator=(C&&);       |
...
如果一个类中既没有自定义的拷贝构造函数，也没有移动构造函数(或者赋值操作符)，编译器会提供一个隐式的拷贝构造函数。该拷贝构造函数只会对类成员进行简单的浅拷贝。

### 拷贝赋值操作符

拷贝赋值操作符的操作数必须是一个左值引用。
```c++
Apple app1(Apple());
...
app4 = app3;
```
上述对 app4 构造时，才会调用 Apple 的拷贝赋值操作符。

对象不是在声明时拷贝构造(只是进行了初始化)，而是在随后的赋值操作中构造。所以当拷贝赋值操作符(copy assignment operator)以 `operator=` 重载的形式出现，需要结合定义来区分是拷贝构造函数还是拷贝赋值操作符，如下:
```c++
MyClass foo;
...
foo = bar;              // 调用拷贝赋值操作符
```
baz 以 = 进行构造的操作是拷贝构造而不是拷贝赋值操作。原因是 baz 在进行声明的同时，又(以 = 的方式)进行了拷贝构造，这里的 = 是拷贝构造的另一种语法表达，它实际是调用了一个单入参形式的拷贝构造函数。而对 foo 的构造调用了拷贝赋值操作符。因为在调用构造函数时，foo 已经作了声明。

类似于拷贝构造函数，如果一个类中既没有自定义的拷贝赋值操作符，也没有移动赋值操作符(或移动构造函数)，编译器会提供一个隐式的浅拷贝方式的拷贝赋值操作符。

由于拷贝构造函数与拷贝赋值操作符定义不同，所以在深拷贝时也会不同。体验下面的代码:
```c++
// 深拷贝时的复制构造函数
class UserString {
private:
  ...
  size_t size = 0;
public:
  UserString(const UserString& rhs) {
    if (rhs.size > 0 && rhs.buffer != nullptr) {
      size = rhs.size;
...
private:
  ...
  size_t size = 0;
public:
  UserString& operator=(const UserString& rhs) {
...
移动构造函数和移动赋值操作符均以同类型对象的右值引用(通常是将亡值)作为参数，但不同于左值拷贝函数(对内存地址对象的拷贝，右值是没有地址的)，右值移动函数是将一个对象(源)转移给另一个对象(目标)，结果是源对象失去了资源，而目标对象接管了相应的资源。

右值移动函数只会在构造函数入参为不具名对象(右值引用)时被调用。

区分是调用移动构造函数还是移动赋值操作符，同样要根据具体定义来进行判断:
```c++
MyClass fn();       // 返回一个函数对象
MyClass foo;        // 缺省构造函数
